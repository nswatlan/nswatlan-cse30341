...
void truncate_content(char *content, int max_width);
...
void display_dir_contents(const char *dir_path) {
    ...
    int min_rows = 4; //2 for header, one for content, one for summary
    int min_cols = 80; //reasonable guess for length of terminal

    if (terminal_rows < min_rows || terminal_cols < min_cols) {
        printf("Not enough space to display contents. Resize your terminal and run the program again.\n");
        exit(1);
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        char truncated_content[BUFSIZ] = ""; //buffer for truncated content
        display_file_details(entry_path, entry, content_width);
        truncate_content(truncated_content, content_width); //truncate as needed
...
void truncate_content(char *content, int max_width) {
    if ((int)strlen(content) > max_width) {
        content[max_width - 3] = '.'; // Add ellipsis at the end
        content[max_width - 2] = '.';
        content[max_width - 1] = '.';
        content[max_width] = '\0';
    }
